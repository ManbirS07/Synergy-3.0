...
// src/module_bindings/add_reducer.ts
var add_reducer_default = {
  name: t.string()
};

// src/module_bindings/say_hello_reducer.ts
var say_hello_reducer_default = {};

// src/module_bindings/person_table.ts
var person_table_default = t.row({
  name: t.string()
});
...
var tablesSchema = schema({
  person: table(
    ...
    person_table_default
  )
});
var reducersSchema = reducers(
  reducerSchema("add", add_reducer_default),
  reducerSchema("say_hello", say_hello_reducer_default)
);
...
var reducers2 = convertToAccessorMap(
...
var DbConnection = class _DbConnection extends DbConnectionImpl {
  ...
  static {
    ...
    this.builder = () => {
      return new DbConnectionBuilder2(
        REMOTE_MODULE,
        (config) => new _DbConnection(config)
      );
    };
  }
};
...
// src/main.ts
var HOST = process.env.SPACETIMEDB_HOST ?? "ws://localhost:3000";
var DB_NAME = process.env.SPACETIMEDB_DB_NAME ?? "basic-ts";
DbConnection.builder().withUri(HOST).withDatabaseName(DB_NAME).onConnect((conn, identity, _token) => {
  ...
  conn.subscriptionBuilder().subscribe(tables.person);
}).onDisconnect(() => {
