...
function clearPinChunkQueue(ctx) {
	const chunkIds = [];
...
function seedPinChunkQueue(ctx, config) {
...
function upsertPinCrackConfig(ctx, config) {
...
const request_pin_work = spacetimedb.reducer((ctx) => {
	markNodeAlive(ctx);
	if (ctx.db.pinCrackConfig.id.find(1)?.pinFound) return;
	for (const inFlight of ctx.db.pinChunkQueue.pin_chunk_queue_by_assigned_node.filter(ctx.sender)) if (inFlight.status === "processing") return;
...
const submit_pin_result = spacetimedb.reducer({
	chunkId: t.u64(),
	foundPin: t.string().optional()
