...
//#region C:/Users/Jaspa/synergy/Synergy-3.0/spacetimedb/node_modules/headers-polyfill/lib/index.mjs
...
//#endregion
//#region C:/Users/Jaspa/synergy/Synergy-3.0/spacetimedb/node_modules/spacetimedb/dist/server/index.mjs
...
//#endregion
//#region C:/Users/Jaspa/synergy/Synergy-3.0/spacetimedb/src/index.ts
...
function clearMatrixChunks(ctx) {
	const chunkIds = [];
	for (const row of ctx.db.matrixChunkQueue.iter()) chunkIds.push(row.chunkId);
	for (const chunkId of chunkIds) ctx.db.matrixChunkQueue.chunkId.delete(chunkId);
}
...
function upsertMatrixJobConfig(ctx, config) {
...
function seedMatrixChunks(ctx, config) {
...
function assignMatrixChunk(ctx) {
	const job = ctx.db.matrixJobConfig.id.find(1);
	if (!job || job.status !== "running") return;
	let inFlightCount = 0;
	for (const inFlight of ctx.db.matrixChunkQueue.matrix_chunk_queue_by_assigned_node.filter(ctx.sender)) if (inFlight.status === "processing") {
		inFlightCount += 1;
		if (inFlightCount >= MAX_INFLIGHT_CHUNKS_PER_NODE) return;
	}
...
function resetMatrixTaskToPending(ctx) {
	clearMatrixChunks(ctx);
	const job = ctx.db.matrixJobConfig.id.find(1);
	if (job) ctx.db.matrixJobConfig.id.delete(job.id);
	const task = ctx.db.task.taskId.find(MATRIX_TASK_ID);
	if (task) ctx.db.task.taskId.update({
		...task,
		requestHelp: false,
		updatedAtMicros: ctx.timestamp.microsSinceUnixEpoch
	});
}
function parseMatrixJson(value, name) {
	let parsed;
	try {
	...
	if (!Array.isArray(parsed) || parsed.length === 0) throw new SenderError(`${name} must be a non-empty 2D array.`);
	const matrix = parsed;
	const firstRow = matrix[0];
	if (!Array.isArray(firstRow) || firstRow.length === 0) throw new SenderError(`${name} rows must be non-empty arrays.`);
	const colCount = firstRow.length;
	const out = [];
	for (let i = 0; i < matrix.length; i += 1) {
		const row = matrix[i];
		if (!Array.isArray(row) || row.length !== colCount) throw new SenderError(`${name} must be rectangular.`);
		const numericRow = [];
		for (let j = 0; j < row.length; j += 1) {
			const cell = Number(row[j]);
			if (!Number.isFinite(cell)) throw new SenderError(`${name} must contain finite numbers.`);
...
function tryFinalizeMatrixJob(ctx) {
	const job = ctx.db.matrixJobConfig.id.find(1);
	if (!job || job.status !== "running") return;
	...
	for (const chunk of ctx.db.matrixChunkQueue.iter()) {
		...
		completed += 1;
		if (!chunk.tileResultJson) continue;
		...
		if (tile.length !== expectedRows) throw new SenderError("Tile result row count mismatch.");
		for (let r = 0; r < tile.length; r += 1) {
			if (tile[r].length !== expectedCols) throw new SenderError("Tile result column count mismatch.");
...
const init = spacetimedb.init((ctx) => {
	...
	upsertTask(ctx, MATRIX_TASK_ID, "matrix_mul", "Matrix Multiplication", true, false);
	const defaultGrid = {
...
const onConnect = spacetimedb.clientConnected((_ctx) => {
	upsertTask(_ctx, MANDELBROT_TASK_ID, "mandelbrot", "Mandelbrot Rendering", true, true);
	upsertTask(_ctx, PIN_TASK_ID, "pin_guess", "PIN Guessing", true, true);
	upsertTask(_ctx, MATRIX_TASK_ID, "matrix_mul", "Matrix Multiplication", true, false);
});
...
const submit_result = spacetimedb.reducer({
	...
	if (task.taskKey === "matrix_mul") {
		if (!resultData) throw new SenderError("Matrix resultData is required.");
		const chunk = ctx.db.matrixChunkQueue.chunkId.find(chunkId);
		if (!chunk) throw new SenderError("Matrix chunk does not exist.");
		if (chunk.status !== "processing") throw new SenderError("Matrix chunk is not in processing state.");
		if (!chunk.assignedNode || chunk.assignedNode.toHexString() !== ctx.sender.toHexString()) throw new SenderError("Matrix chunk is assigned to another node.");
		ctx.db.matrixChunkQueue.chunkId.update({
			...chunk,
			status: "completed",
...
const submit_matrix_job = spacetimedb.reducer({
	matrixAJson: t.string(),
	matrixBJson: t.string(),
	tileSize: t.u32()
	...
	if (aCols !== bRows) throw new SenderError("A.columns must equal B.rows.");
	if (aRows > 1e3 || aCols > 1e3 || bCols > 1e3) throw new SenderError("Current limit: max matrix dimension is 1000.");
...
//#endregion
export { spacetimedb as default, heartbeat, init, onConnect, onDisconnect, request_work, reset_grid, reset_pin_crack, reset_task, set_task_active, set_task_help, submit_matrix_job, submit_result };
//# debugId=54632944-9759-4c0d-8773-4f6beea2338a
//# sourceMappingURL=data:application/json;charset=utf-8;base64,